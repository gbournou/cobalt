...
void SetHeader(net::HttpRequestHeaders& request_headers,
               const std::string& header, const std::string& value) {
  if (!value.empty()) {
    request_headers.SetHeader(base::StrCat({kCHPrefix, header}), value);
  }
}
...
net::HttpRequestHeaders GetClientHintHeaders(
    const UserAgentPlatformInfo& platform_info) {
  net::HttpRequestHeaders headers;

  // Android OS Experience
  SetHeader(headers, "Android-OS-Experience",
